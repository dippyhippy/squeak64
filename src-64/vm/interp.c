/* Automatically generated from Squeak on #(29 October 2004 2:50:54 pm) */
...
sqInt okayOop(sqInt oop);
sqInt oopFromChunk(sqInt chunk);
sqInt oopHasAcceptableClass(sqInt oop);
sqInt oopHasOkayClass(sqInt oop);
...
sqInt positive64BitIntegerFor(squeakInt64 integerValue);
squeakInt64 positive64BitValueOf(sqInt oop);
...
sqInt printUnbalancedStackFromNamedPrimitive(void);
...
sqInt rewriteMethodCacheSelclassprimIndexprimFunction(sqInt selector, sqInt class, sqInt localPrimIndex, sqInt localPrimAddress);
...
sqInt signed64BitIntegerFor(squeakInt64 integerValue);
squeakInt64 signed64BitValueOf(sqInt oop);
...
register struct foo * foo = &fum;
	...
	formatOfClass = (longAt((classPointer + BaseHeaderSize) + (InstanceSpecificationIndex << ShiftForWord))) - 1;
	header1 = (formatOfClass & 130816) | ((hash << HashBitsOffset) & HashBits);
	header2 = classPointer;
	header3 = 0;
	sizeHiBits = ((usqInt) (formatOfClass & 393216)) >> 9;
	...
	byteSize = (formatOfClass & (SizeMask + Size4Bit)) + sizeHiBits;
	byteSize = byteSize << (ShiftForWord - 2);
	format = (((usqInt) formatOfClass) >> 8) & 15;
	flag("sizeLowBits");
	if (format < 8) {
		if (format == 6) {
			bm1 = BytesPerWord - 1;
...
l2:	/* end lengthOf: */;
	...
	value = 0;
	for (i = 0; i <= (sz - 1); i += 1) {
		value += (((squeakInt64) (byteAt((oop + BaseHeaderSize) + i)))) << (i * 8);
	}
	return value;
}
...
register struct foo * foo = &fum;
	...
	foo->newMethod = longAt((blockClosure + BaseHeaderSize) + (BlockMethodIndex << ShiftForWord));
	/* begin primitiveIndexOf: */
	primBits = (((usqInt) (longAt((foo->newMethod + BaseHeaderSize) + (HeaderIndex << ShiftForWord)))) >> 1) & 268435967;
	foo->primitiveIndex = (primBits & 511) + (((usqInt) primBits) >> 19);
	/* begin success: */
	foo->successFlag = (foo->argumentCount == ((((usqInt) (longAt((foo->newMethod + BaseHeaderSize) + (HeaderIndex << ShiftForWord)))) >> 25) & 15)) && foo->successFlag;
	if (foo->successFlag) {
...
register struct foo * foo = &fum;
	...
	blockClosure = longAt(foo->stackPointer - (1 * BytesPerWord));
	/* begin push: */
	longAtput(sp = foo->stackPointer + BytesPerWord, longAt((blockClosure + BaseHeaderSize) + (BlockMethodIndex << ShiftForWord)));
	foo->stackPointer = sp;
...
register struct foo * foo = &fum;
	...
	/* begin success: */
	successValue1 = ((fetchClassOf(lit)) == (longAt((foo->specialObjectsOop + BaseHeaderSize) + (ClassArray << ShiftForWord)))) && ((lengthOf(lit)) == 4);
	foo->successFlag = successValue1 && foo->successFlag;
	if (!(foo->successFlag)) {
		return null;
	}
	...
	if ((0 >= -1073741824) && (0 <= 1073741823)) {
		/* begin storePointer:ofObject:withValue: */
		if (lit < foo->youngStart) {
			possibleRootStoreIntovalue(lit, ((0 << 1) | 1));
		}
		longAtput((lit + BaseHeaderSize) + (2 << ShiftForWord), ((0 << 1) | 1));
	} else {
		/* begin primitiveFail */
		foo->successFlag = 0;
	}
	...
	if ((0 >= -1073741824) && (0 <= 1073741823)) {
		...
		longAtput((lit + BaseHeaderSize) + (3 << ShiftForWord), ((0 << 1) | 1));
	} else {
		/* begin primitiveFail */
		foo->successFlag = 0;
	}
...
l2:	/* end fetchClassOf: */;
	...
	if (foo->successFlag) {
		...
		for (i1 = 0; i1 <= foo->argumentCount; i1 += 1) {
			longAtput(where + ((ReceiverIndex + i1) << ShiftForWord), longAt(foo->stackPointer - ((foo->argumentCount - i1) * BytesPerWord)));
		}
		nilOop = foo->nilObj;
		...
		/* begin storePointer:ofObject:withValue: */
		valuePointer11 = (((((((usqInt) ((foo->stackPointer - foo->activeContext) - BaseHeaderSize)) >> ShiftForWord) - TempFrameStart) + 1) << 1) | 1);
		if (activeCntx < foo->youngStart) {
			possibleRootStoreIntovalue(activeCntx, valuePointer11);
		}
		longAtput((activeCntx + BaseHeaderSize) + (StackPointerIndex << ShiftForWord), valuePointer11);
		if (newContext < foo->youngStart) {
			beRootIfOld(newContext);
		}
		foo->activeContext = newContext;
...
register struct foo * foo = &fum;
	...
	moduleName = longAt(foo->stackPointer - (0 * BytesPerWord));
	if ((moduleName & 1)) {
		/* begin success: */
		foo->successFlag = 0 && foo->successFlag;
		return null;
...
/*	Behaviour depends on argument count:
	...
	VM parameters are numbered as follows:
		...
		26	BytesPerWord for this image
...
register struct foo * foo = &fum;
	...
	if (foo->argumentCount == 0) {
		...
		longAtput((result + BaseHeaderSize) + (25 << ShiftForWord), ((BytesPerWord << 1) | 1));
		/* begin pop:thenPush: */
		longAtput(sp = foo->stackPointer - ((1 - 1) * BytesPerWord), result);
		foo->stackPointer = sp;
		return null;
...
register struct foo * foo = &fum;
	...
	lit = longAt((foo->newMethod + BaseHeaderSize) + ((0 + LiteralStart) << ShiftForWord));
	printStringOf(longAt((lit + BaseHeaderSize) + (1 << ShiftForWord)));
	print(" in ");
	printStringOf(longAt((lit + BaseHeaderSize) + (0 << ShiftForWord)));
	/* begin cr */
	printf("\n");
}
...
register struct foo * foo = &fum;
	...
	bytesRead = sqImageFileRead(pointerForOop((sqInt)memory), sizeof(unsigned char), dataSize, f);
